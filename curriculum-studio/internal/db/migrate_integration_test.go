...
func TestP1E1_FreshMigrateFullInventory(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 6, n, "expected 6 applied goose versions")
...
func TestP1E5_DownPolicyLiveVsNonLive(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, int64(5), v, "down should roll back exactly one version (00006)")
