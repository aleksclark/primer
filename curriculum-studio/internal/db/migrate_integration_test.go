...
func TestP1E1_FreshMigrateFullInventory(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 8, n, "expected 8 applied goose versions")
...
func TestP1E5_DownPolicyLiveVsNonLive(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, int64(7), v, "down should roll back exactly one version (00008)")
