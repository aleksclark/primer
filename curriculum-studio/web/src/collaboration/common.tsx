...
export function Pager({offset,total,onChange,label='results'}:{offset:number;total:number;onChange:(n:number)=>void;label?:string}){return <nav className="collab-actions" aria-label={`${label} pages`}><button className="secondary" disabled={offset===0} onClick={()=>onChange(Math.max(0,offset-10))}>Previous {label}</button><span>{Math.min(offset+1,total)}–{Math.min(offset+10,total)} of {total}</span><button className="secondary" disabled={offset+10>=total} onClick={()=>onChange(offset+10)}>Next {label}</button></nav>}
