...
export default function CollaborationPane({workspaceId,role,curriculum,revision,onRevision}:{workspaceId:string;role:string;curriculum:Model<'Curriculum'>;revision:Model<'PlanRevision'>;onRevision:(r:Model<'PlanRevision'>)=>void}){
 ...
 const page=useRemote(signal=>revisions(curriculum.id,offset,signal),[curriculum.id,offset,revision.id]);
...
function RevisionContents({revision,workspaceId,role,refreshMetadata}:{revision:Model<'PlanRevision'>;workspaceId:string;role:string;refreshMetadata:()=>void}){
 ...
 async function create(){setSaving(true);setMessage('');try{await addNode(revision.id,kind,title);setTitle('');changed();setMessage('Plan node created. Any old review must be renewed.')}catch(e){setMessage(problem(e))}finally{setSaving(false)}}
 ...
 {editable&&<form className="collab-form" onSubmit={e=>{e.preventDefault();void create()}}><label htmlFor="node-kind">New node kind</label><select id="node-kind" value={kind} onChange={e=>setKind(e.target.value==='unit'?'unit':'outcome')}><option value="outcome">Outcome</option><option value="unit">Unit</option></select><label htmlFor="node-title">Name</label><input id="node-title" required value={title} onChange={e=>setTitle(e.target.value)}/><button className="secondary" disabled={saving}>Add node</button></form>}{message&&<p role="status">{message}</p>}</section>
