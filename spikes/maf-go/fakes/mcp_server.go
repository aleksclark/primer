...
type MCPEnv struct {
	...
	// BlockAllow makes allow_me wait until ctx done or timeout.
	BlockAllow time.Duration
	mu         sync.Mutex
}

// StartMCP starts in-memory MCP with allow_me and deny_me tools.
func StartMCP(t *testing.T) *MCPEnv {
	t.Helper()
	env := &MCPEnv{}
	server := mcp.NewServer(&mcp.Implementation{Name: "spike-mcp", Version: "1.0.0"}, nil)
...
func (e *MCPEnv) AllTools(t *testing.T) []tool.Tool {
	t.Helper()
