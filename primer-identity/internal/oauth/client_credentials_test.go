...
func TestClientCredentialsIssuesServiceTokenWithoutRefreshOrProvider(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `INSERT INTO oauth_service_credentials(service_principal_id,oauth_client_id,secret_hash,pepper_version,allowed_resources,allowed_audiences,allowed_scopes) VALUES($1,$2,$3,1,$4,$5,$6)`, principal, client, hash, []string{"https://resource.example"}, []string{"curriculum-studio"}, []string{"read", "draft"})
	require.NoError(t, err)

	svc := newTestService(t, secrets, frozenClock{now: time.Date(2026, 8, 18, 12, 0, 0, 0, time.UTC)})
	resp, err := svc.Exchange(ctx, oauth.ExchangeRequest{GrantType: oauth.GrantClientCredentials, Resource: "https://resource.example", Scope: "read"}, oauth.ClientAuth{Method: oauth.AuthBasic, ClientID: clientID, Secret: plain})
	require.NoError(t, err)
	...
	require.Equal(t, "read", payload["scope"])
