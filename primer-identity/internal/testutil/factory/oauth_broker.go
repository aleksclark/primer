...
func BrokerTransaction(t *testing.T, q repo.Querier, c *domain.OAuthClient, r *domain.OAuthClientRedirect) *domain.BrokerTransaction {
	t.Helper()
	now := time.Now().UTC()
	v, err := repo.CreateBrokerTransaction(context.Background(), q, domain.CreateBrokerTransactionInput{OAuthClientID: c.ID, RedirectID: r.ID, StateHash: []byte(strings.Repeat("s", 32)), StatePepperVersion: 1, StateSealed: []byte(strings.Repeat("x", 30)), StateKeyVersion: 1, StateLength: 1, PKCEChallenge: strings.Repeat("A", 43), PKCEMethod: "S256", RequestedScopes: []string{"openid"}, ResourceURI: r.ResourceURI, Audience: r.Audience, BrokerCookieHash: []byte(strings.Repeat("c", 32)), BrokerCookiePepperVersion: 1, CreatedAt: now, ExpiresAt: now.Add(time.Minute)})
	if err != nil {
		t.Fatal(err)
	}
	return v
