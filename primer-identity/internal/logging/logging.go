...
func redactAnyValue(v any) string {
	...
	case fmt.Stringer:
		// Prefer Stringer over default fmt for types that implement both
		// (errors usually implement Error() already handled above).
...
func redactSecretText(s string) string {
	...
	// Whole-value DSN (common for database_url attrs).
	if looksLikeDSN(s) && !strings.ContainsAny(s, " \t\n") {
		return redactDSN(s)
	}
	// Embedded URIs inside error/message text.
...
func redactDSN(raw string) string {
	u, err := url.Parse(raw)
	if err != nil || u.Scheme == "" {
		// Best-effort: strip userinfo between scheme:// and @.
