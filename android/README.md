...
- Native SDK is Clerk Android API **0.1.31** because 1.1.x ships Kotlin 2.4 metadata. Hosted Account Portal is not in 0.1.31. Control implements official `SignIn.create` password and official Google OAuth (`SignIn.authenticateWithRedirect` / `OAuthProvider.GOOGLE`), then continues Clerk-supported second factors on the same SignIn object: TOTP, backup code, SMS, or email code (`attemptSecondFactor` / `prepareSecondFactor`). Only the **new** session ID is activated. Google sign-up without an existing parent is incomplete, not household creation. Unsupported incomplete states stay incomplete. Do not weaken server policy.
