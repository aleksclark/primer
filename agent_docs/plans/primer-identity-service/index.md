...
**Cursor:** **IB4 / I8 signed webhook and two-plane revocation**. The IB0 exact-tip review passed at `4bfd6d5c03412d134a635c32279ef37b1c4e0d9e` (0 Critical/Important/Minor), so Phase 08's former STOP is cleared; implementation may proceed from current master. The residual IB3 browser E2E remains explicitly unproven.

**Post-merge roadmap cursor:** **IB4 / I8 signed webhook and two-plane revocation**. I7's credential-free BFF contract is complete, but production BFF/MCP authorization remains blocked until the fresh IB4 exact-tip specification/security gate passes. Live Stytch browser/webhook proof remains **BLOCKED**.
...
| [08: IB4 / signed webhook and two-plane revocation](./phase-08-service-principals.md) | **Implementation present in PR #37 (`313ae56`), review pending.** Make provider invalidation and local grants durable and replay safe, with four reason-bound collision classes and one restart-safe alert. | IB1–IB3; hard gate for production BFF/MCP. |
