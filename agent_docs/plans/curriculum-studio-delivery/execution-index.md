...
**IB2/I6 status: COMPLETE — dual-reviewed hardening tip `f5d5b5b372dc6988a65d082f948b96e7de9f432b` (+ live test harness `e8ecd18`).** Spec PASS 0C/0I; quality/security APPROVED 0C/0I. Supersedes earlier master tip `f31559d` (PR #24). Full IB8-E10 browser/webhook remains **BLOCKED**.
...
| **I6** | IB2 / Primer ES256 JWT and JWKS bridge — **complete and dual-reviewed** at hardening tip `f5d5b5b` (supersedes master `f31559d`); harness `e8ecd18` | Identity | [phase-06](../primer-identity-service/phase-06-google-rp-loopback.md); [resume](../identity-ib2-resume.md) | I5 | PG5b | `impl/IB2-hardening-remed` (prior `impl/IB2-primer-jwks` merged as PR #24) | **Green on `f5d5b5b`:** identity-test-oauth, identity-e2e, identity-cover 82.4%, identity-openapi, foundation-check; IB2-E00..E11 + hardening (host-rooted aud, 60s assertion retention, ClientLookup, exact invalid_client, auth-code fields, 00008/00009); opt-in identity-live-stytch test-project only | Full IB8-E10 **BLOCKED**; next I7 |
| **I7** | IB3 / BFF cookie, CSRF and PKCE contract — **complete on master via PR #26 (`81b3302`)** for the credential-free package; browser E01..E06 residual | Identity | [phase-07](../primer-identity-service/phase-07-product-bff-contract.md) | I5, I6 | PG6 | `impl/I7-bff-contract` | package tests green; real-browser/process/artifact proof still open | **milestone:** production BFF remains gated by I8 |
| **I8** | IB4 / signed webhook and two-plane revocation — **merged PR #37 (`b2f4ae4`); exact-tip review pending** | Identity | [phase-08](../primer-identity-service/phase-08-service-principals.md) | I5, I6, **I7** | PG6b | `impl/I8-webhook-revocation` | Credential-free implementation tests green; IB4-E01..E11 exact-tip review remains required for receipt/collision/security-alert/revocation/audit migrations, indexes, leases, delete actions and 400-day retention; signed replay/forgery/dedupe/out-of-order, lease/backoff, four reason-bound collision classes, concurrent one-event/observation/alert, and no authority effect | **milestone:** hard production BFF/MCP revocation gate |
| **I9** | IB5 / Primer-owned service principals and `client_credentials` — **implementation at `ce3c17c`; review pending** | Identity | [phase-09](../primer-identity-service/phase-09-refresh-revoke-logout.md) | I6 | PG7 | `impl/I9-service-principals` | Credential-free service principal/client_credentials implementation and class/scope/audience proof; fresh exact-tip security review required | **milestone:** S15 machine JWT |
| **I10** | IB6 / provider-plus-Primer lifecycle — **current implementation wave** | Identity | [phase-10](../primer-identity-service/phase-10-key-rotation-and-hardening.md) | I7, I8 | PG8 | `impl/I10-lifecycle` | Credential-free refresh rotation/reuse terminalization, scope narrowing, no-refresh service path, and exact OAuth form/auth proof; fresh exact-tip lifecycle review still required | — |
| **I11** | IB7 / key rotation and hardening — **current implementation wave** | Identity | [phase-11](../primer-identity-service/phase-11-admin-audit-recovery.md) | I6, I8 | PG8 | `impl/I11-hardening` | Credential-free next→active→retired→destroyed rotation with unique active/next and ciphertext destruction proof; fresh exact-tip review still required | — |
| **I12** | IB8 / Studio local-authorization proof — **current implementation wave** | Identity (+ Platform consumer) | [phase-12](../primer-identity-service/phase-12-studio-integration.md); Platform S2 | I6, I8, S2 | PG9 | `impl/I12-studio-integration` | Credential-free Studio validator proof for signed public `client_id`, service/human class, local membership, missing/wrong `client_id`, `azp`, internal UUID, and raw-token denial; live promotion remains BLOCKED | **milestone:** applicable production validator/cutover chain |
...
| Wave | Goal | Owner | Detailed refs | Depends on | PG | Branch | Acceptance | Stop |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **D1** | Migration freeze + Studio migrator lifecycle — **complete** | Database | [phase-01](../curriculum-studio-database/phase-01-migration-freeze-and-lifecycle.md) | F0 | **PG1** | `impl/D1-migration-freeze` | G-db-py; migrate up/down policy — green on merged history | no root Makefile race |
| **D2** | pgx pool, Querier/UoW, testcontainers harness — **complete** | Database | [phase-02](../curriculum-studio-database/phase-02-persistence-foundation.md) | D1 | PG1 | `impl/D2-persistence-foundation` | G-studio-test (persistence pkgs) — green on merged history | no in-memory prod repos |
| **D3** | Tenant/workspace/membership projections — **complete** | Database | [phase-03](../curriculum-studio-database/phase-03-tenant-workspace-authz.md) | D2 | PG2 | `impl/D3-authz-projections` | G-studio-test — green on merged history | — |
| **D4** | Catalogs + resources repos — **complete in PR #30** | Database | [phase-04](../curriculum-studio-database/phase-04-catalogs-and-resources.md) | D3 | PG3b | `impl/D4-catalogs` | G-studio-test; real-Postgres catalog/resource scope, ownership, cycle, and payload-policy evidence — merged at `710e6ff` | — |
| **D5** | Plan graph + publish consistency — **complete** at `cc870c3` | Database | [phase-05](../curriculum-studio-database/phase-05-plan-graph-and-publication.md) | D4 | PG4b | `impl/D5-plan-graph` | G-studio-test; P5-E1–P5-E5 with real PostgreSQL and concurrent cycle contention — green | — |
| **D6** | Validation reports persistence — **complete** at `bbae7fe` | Database | [phase-06](../curriculum-studio-database/phase-06-validation-reports.md) | D5 | PG5c | `impl/D6-validation-reports` | G-studio-test; P6-E1–P6-E2 with real PostgreSQL — green | parallel D7 OK |
...
| **C5** | gRPC integration service harness — **complete** (PR #31, `b35164d`) | Contracts | [phase-05](../curriculum-studio-contracts/phase-05-grpc-integration-harness.md) | C4 | PG4 | `impl/C5-grpc-harness` | generated client E2E | stubs only OK |
| **C6** | Huma boundary DTOs + offline OpenAPI emission — **complete** (PR #36, `eb2fd8c`) | Contracts | [phase-06](../curriculum-studio-contracts/phase-06-huma-openapi-emission.md) | C2–C3; needs S1 handler host hooks | PG3 / after S1 | `impl/C6-huma-emit` | openapi-gen offline | may stub handlers |
| **C7** | OpenAPI baseline handoff + REST clients — **complete** (PR #41, `656d8a1`) | Contracts | [phase-07](../curriculum-studio-contracts/phase-07-openapi-handoff-and-rest-clients.md) | C6 | PG5d | `impl/C7-openapi-clients` | TS/Go clients gen | stop dual SoT |
| **C8** | Auth/errors/idempotency/pagination semantics — **complete** (PR #44, `2310fcf`) | Contracts | [phase-08](../curriculum-studio-contracts/phase-08-auth-errors-idempotency-pagination.md) | C5, C7 | PG6c | `impl/C8-auth-errors` | REST+gRPC semantics | X-Service-Token alias only |
| **C9** | Domain events + webhook envelope conformance — **complete** (PR #46, `2e55dd3`) | Contracts | [phase-09](../curriculum-studio-contracts/phase-09-events-webhooks.md) | C5, C8 | PG7c | `impl/C9-events` | envelope tests | — |
| **C10** | Compatibility + exclusive-use + clean-checkout gates — **current** | Contracts | [phase-10](../curriculum-studio-contracts/phase-10-compatibility-and-policy-gates.md) | C4–C9 | PG8c | `impl/C10-policy-gates` | planted reds | — |
...
Historical F0/PG1/PG2 are complete history; do not redispatch I1/I2. IA foundation `87d5c215`; IA-R `8623ee63`; IB0 design `4bfd6d5c`; IB1 `59a39982`; IB2 on master `f31559d` (PR #24); **IB2 dual-reviewed hardening tip `f5d5b5b` (+ harness `e8ecd18`) is local on `impl/IB2-hardening-remed` and is the dual-reviewed tip; next is IB3.** Ordering: IB3 BFF → IB4 webhook/revocation → IB5 service principals → IB6 lifecycle → IB7 key rotation → IB8 Studio/LMS/TV/live. Full IB8-E10 browser/webhook remains **BLOCKED**. Resume: [`../identity-ib2-resume.md`](../identity-ib2-resume.md).
