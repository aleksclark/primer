...
**Contracts status:** C1–C7 are complete on current master; C4 landed in PR #28 (`db587ed`), C5 in PR #31 (`b35164d`), C6 in PR #36 (`eb2fd8c`), C7 in PR #41 (`656d8a1`). **Contracts cursor: C8** (auth, errors, idempotency, and pagination semantics).
...
| **C8** | Auth/errors/idempotency/pagination semantics — **current** | Contracts | [phase-08](../curriculum-studio-contracts/phase-08-auth-errors-idempotency-pagination.md) | C5, C7 | PG6c | `impl/C8-auth-errors` | REST+gRPC semantics | X-Service-Token alias only |
