...
**IB2/I6 status: COMPLETE — dual-reviewed hardening tip `0dd5faac34da7afc19a11c3e463d7d28eef2b538` (+ live test harness `bee0a5c`).** Spec PASS 0C/0I; quality/security APPROVED 0C/0I. Master still at earlier reviewed tip `f31559d` (PR #24) until hardening is integrated. Full IB8-E10 browser/webhook remains **BLOCKED**.
...
**Current status:** IB2/I6 dual-reviewed at `0dd5faa` on local branch `impl/IB2-hardening-remed`; coverage 82.4% (≥80%); parent gates green; opt-in `make identity-live-stytch` green against Stytch test project (not IB8-E10). Resume handoff: [`../identity-ib2-resume.md`](../identity-ib2-resume.md).
**Post-merge cursor:** **(1) Integrate IB2 hardening tip to `master`, then (2) PROCEED to IB3 / I7 BFF cookie/CSRF/PKCE.** Do not claim production BFF or MCP authorization from this status.
...
| **I6** | IB2 / Primer ES256 JWT and JWKS bridge — **complete and dual-reviewed** at hardening tip `0dd5faa` (master still `f31559d` until integrated); harness `bee0a5c` | Identity | [phase-06](../primer-identity-service/phase-06-google-rp-loopback.md); [resume](../identity-ib2-resume.md) | I5 | PG5b | `impl/IB2-hardening-remed` (prior `impl/IB2-primer-jwks` merged as PR #24) | **Green on `0dd5faa`:** identity-test-oauth, identity-e2e, identity-cover 82.4%, identity-openapi, foundation-check; IB2-E00..E11 + hardening (host-rooted aud, 60s assertion retention, ClientLookup, exact invalid_client, auth-code fields, 00008/00009); opt-in identity-live-stytch test-project only | Full IB8-E10 **BLOCKED**; integrate hardening before I7 |
| **I7** | IB3 / BFF cookie, CSRF and PKCE contract — **next after I6 hardening on master** | Identity | [phase-07](../primer-identity-service/phase-07-product-bff-contract.md) | I5, I6 | PG6 | `impl/I7-bff-contract` | G-identity-e2e | **milestone:** Studio BFF OIDC; do not start from master `f31559d` alone |
...
Historical F0/PG1/PG2 are complete history; do not redispatch I1/I2. IA foundation `87d5c215`; IA-R `8623ee63`; IB0 design `4bfd6d5c`; IB1 `59a39982`; IB2 on master `f31559d` (PR #24); **IB2 dual-reviewed hardening tip `0dd5faa` (+ harness `bee0a5c`) is local on `impl/IB2-hardening-remed` and must integrate before IB3.** Ordering: IB3 BFF → IB4 webhook/revocation → IB5 service principals → IB6 lifecycle → IB7 key rotation → IB8 Studio/LMS/TV/live. Full IB8-E10 browser/webhook remains **BLOCKED**. Resume: [`../identity-ib2-resume.md`](../identity-ib2-resume.md).
