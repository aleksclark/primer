...
export default function AgentCommandPage() {
  ...
  const client = useMemo(() => conversationId ? tasksClient.createParentAgentClient(conversationId) : null, [conversationId]);
  ...
  return <><PageHeader eyebrow="Parent workspace / Command + Inspect" title="Parent agent" lede="A bounded command surface for inspecting and preparing Tasks changes. The ordinary task and schedule pages remain the source of truth." actions={<><ConnectionState snapshot={snapshot} />{active && <button className="button danger" type="button" onClick={() => sendWrappedClient.cancel(snapshot.runId)}>Cancel run</button>}</>} />
    ...
    <div className="agent-layout"><div><AgentTranscript items={items} client={sendWrappedClient} snapshot={snapshot} /><AgentComposer client={sendWrappedClient} disabled={disabled || active} /></div><aside className="agent-inspector" aria-label="Agent run boundaries"><p className="eyebrow">Inspect / boundaries</p><dl><div><dt>Transport</dt><dd>Authenticated socket</dd></div><div><dt>Cursor</dt><dd className="meta">{snapshot.cursor || "—"}</dd></div><div><dt>Run state</dt><dd>{status ? status.replaceAll("_", " ") : "No active run"}</dd></div><div><dt>Authority</dt><dd>Server-owned Tasks and Schedules</dd></div></dl><p className="meta">Thinking is shown only as a generic state. Tool activity uses an allowlisted label; provider reasoning and raw tool arguments never render here.</p></aside></div>
  </>;
}
