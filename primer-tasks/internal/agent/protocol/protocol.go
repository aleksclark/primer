...
func Schema() map[string]any {
	return map[string]any{
		"protocol":    Version,
		"commands":    []string{"hello", "subscribe", "unsubscribe", "user_message", "cancel", "confirm", "ack"},
		"events":      []string{"user_message", "hello", "text_start", "text_delta", "text_end", "thinking_start", "thinking_end", "tool_progress", "confirmation_required", "retry", "terminal", "error", "replay_gap"},
		"eventFields": map[string][]string{"user_message": {"text", "clientMessageId"}, "text_delta": {"text"}, "tool_progress": {"label", "phase"}, "confirmation_required": {"confirmationId", "summary", "expiresAt"}, "retry": {"retry", "retryAfterMs"}, "terminal": {"status"}, "error": {"code"}},
	}
}
