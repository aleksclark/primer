...
type PostgresRepository struct{ DB tasksdb.Database }

func NewPostgresRepository(db tasksdb.Database) *PostgresRepository {
...
func (r *PostgresRepository) TransitionRun(ctx context.Context, tenant, id string, to RunStatus, step int, u Usage) error {
	...
	if _, err = tx.Exec(ctx, `UPDATE agent_runs SET status=$3,durable_step=GREATEST(durable_step,$4),input_tokens=GREATEST(input_tokens,$5),output_tokens=GREATEST(output_tokens,$6),total_tokens=GREATEST(total_tokens,$7),reasoning_tokens=GREATEST(reasoning_tokens,$8),updated_at=now() WHERE tenant_id=$1 AND id=$2`, tenant, id, to, step, u.InputTokens, u.OutputTokens, u.TotalTokens, u.ReasoningTokens); err != nil {
		return err
	}
