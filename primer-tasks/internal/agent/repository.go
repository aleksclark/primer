...
func (r *PostgresRepository) GetRun(ctx context.Context, tenant, id string) (run Run, err error) {
	err = r.DB.QueryRow(ctx, `SELECT id,tenant_id,conversation_id,user_message_id,status,durable_step,attempt,max_steps,max_tokens,deadline,lease_owner,lease_until,cancel_requested,input_tokens,output_tokens,total_tokens,reasoning_tokens,provider,model,policy_version,prompt_digest,created_at,updated_at FROM agent_runs WHERE tenant_id=$1 AND id=$2`, tenant, id).Scan(&run.ID, &run.TenantID, &run.ConversationID, &run.UserMessageID, &run.Status, &run.DurableStep, &run.Attempt, &run.MaxSteps, &run.MaxTokens, &run.Deadline, &run.LeaseOwner, &run.LeaseUntil, &run.CancelRequested, &run.Usage.InputTokens, &run.Usage.OutputTokens, &run.Usage.TotalTokens, &run.Usage.ReasoningTokens, &run.Provenance.Provider, &run.Provenance.Model, &run.Provenance.PolicyVersion, &run.Provenance.PromptDigest, &run.CreatedAt, &run.UpdatedAt)
	if err == pgx.ErrNoRows {
		err = ErrNotFound
	}
	return
