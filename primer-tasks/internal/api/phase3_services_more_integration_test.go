...
func TestPhase3ServiceAdaptersCoverTaskScheduleAndOccurrencePaths(t *testing.T) {
	...
	updated, err := services.UpdateSchedule(ctx, scope, parent.ScheduleUpdateInput{ScheduleID: sch.ID, ScheduleInput: parent.ScheduleInput{Timezone: "America/Chicago", StartAt: start.Add(time.Hour), DueOffsetMinutes: 7}})
	...
	if err != nil || len(bulk) != 1 || bulk[0].Enabled {
