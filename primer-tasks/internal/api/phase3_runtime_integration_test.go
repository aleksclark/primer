...
func TestDurableToolEffectLedgerReplaysAndRejectsChangedMutation(t *testing.T) {
	...
	if replay, err := svc.beginToolEffect(ctx, parent.ServiceContext{RunID: "test-run"}, parent.ToolDraftTask, map[string]string{"title": "unit"}); !errors.Is(err, parent.ErrInvalidInput) || replay != nil {
		t.Fatalf("invalid durable ledger=%v %v", replay, err)
	}
	if err := svc.completeToolEffect(ctx, parent.ServiceContext{RunID: "test-run"}, parent.ToolDraftTask, nil, map[string]string{}); !errors.Is(err, parent.ErrInvalidInput) {
		t.Fatal(err)
	}
	...
	if _, err = svc.PublishTask(ctx, toolCtx, first.ID); err == nil {
		t.Fatal("terminal run allowed an effect")
	}
	runID = seedAgentTestRun(t, New(pool, "test"), tenantA, "parent-a")
	toolCtx.RunID = runID
	// A fresh authorized run has the same replay and changed-input rules.
...
func TestPostgresAgentRepositoryTransitionsLeasesReplayRestartAndConfirm(t *testing.T) {
	...
	if _, _, err := store.AppendUserMessage(ctx, agent.Message{ID: messageID, TenantID: tenant, ConversationID: conversationID, ClientMessageID: "restart-message", Role: agent.RoleUser, Content: "restart", Sequence: 3, CreatedAt: now}); err != nil {
		t.Fatal(err)
	}
