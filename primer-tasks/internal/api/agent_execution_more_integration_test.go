...
func TestScriptedAgentExecutionUsesServerToolsAndPersistsSafeProgress(t *testing.T) {
	...
	if err != nil || run.Status != agent.RunSucceeded || run.DurableStep != 1 {
...
func TestFantasyToolAdaptersExecuteTargetedMutation(t *testing.T) {
	...
	runtime, err := agent.NewFantasyAgent(model, s.fantasyTools(tenantA, "parent-a", defaultToolNames(), "digest"), agent.Limits{MaxSteps: 3, MaxTokens: 1000, Deadline: time.Second})
	if err != nil {
		t.Fatal(err)
	}
	if _, err = runtime.Execute(ctx, "targeted-run", "publish", func(agentprotocol.Event) error { return nil }); err != nil {
		t.Fatal(err)
	}
...
func TestFantasyToolAdaptersCoverReadAndPreviewTools(t *testing.T) {
	...
	createInput := fmt.Sprintf(`{"studentId":%q,"templateId":%q,"revisionId":%q,"kind":"one_off","timezone":"UTC","startAt":%q,"dueOffsetMinutes":0}`, alice, published.TemplateID, published.ID, time.Now().UTC().Add(time.Hour).Format(time.RFC3339))
	for _, tc := range []struct{ name, input string }{
		{"list_schedules", `{"includeDisabled":true,"limit":10}`},
		{"list_occurrences", `{"status":"","limit":10}`},
		{"preview_action", `{"kind":"retire_task","targetIds":["00000000-0000-0000-0000-000000000001"],"payload":{"expectedVersion":1},"summary":"Preview only"}`},
		...
		runtime, err := agent.NewFantasyAgent(model, s.fantasyTools(tenantA, "parent-a", defaultToolNames(), "digest"), agent.Limits{MaxSteps: 2, MaxTokens: 1000, Deadline: time.Second})
		if err != nil {
			t.Fatal(err)
		}
		if _, err = runtime.Execute(ctx, "read-"+tc.name, tc.name, func(agentprotocol.Event) error { return nil }); err != nil {
