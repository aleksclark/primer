...
func TestPhase3DomainAdaptersCoverScheduleAndRevisionBoundaries(t *testing.T) {
	...
	if _, err = adapters.UpdateSchedule(ctx, scope, parent.ScheduleUpdateInput{ScheduleID: sch.ID, ScheduleInput: parent.ScheduleInput{Timezone: "America/Chicago", StartAt: start, RRULE: ""}}); err != nil {
		t.Fatal(err)
	}
