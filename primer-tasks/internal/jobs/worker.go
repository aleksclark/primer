...
func (r *PostgresRepository) Renew(ctx context.Context, id, owner string, lease time.Duration) error {
	var renewed bool
	if err := r.DB.QueryRow(ctx, `UPDATE agent_jobs SET lease_until=now()+$3::interval,updated_at=now() WHERE id=$1 AND status='running' AND lease_owner=$2 AND lease_until>now() RETURNING true`, id, owner, fmt.Sprintf("%f seconds", lease.Seconds())).Scan(&renewed); err != nil {
		return err
	}
	if !renewed {
...
func (w *Worker) RunsTransition(ctx context.Context, j Job, status agent.RunStatus) {
	if w.Runs != nil {
		if run, err := w.Runs.GetRun(ctx, j.TenantID, j.RunID); err == nil && (run.Status.Terminal() || run.Status == agent.RunAwaitingConfirmation) {
			return
		}
