...
func TestDialogueMigrationsPreserveReleasedP3AndFenceEvidence(t *testing.T) {
	...
	execDialogueSQL(t, ctx, pool, `INSERT INTO verification_overrides(id,tenant_id,occurrence_id,requirement_id,attempt_id,client_request_id,expected_version,result_version,result_status,accepted,reason,actor_id) VALUES($1,$2,$3,$4,$5,'override-one',4,5,'pending',true,'Parent observed the work.','local-parent')`, override, f.tenant, f.occurrence, f.requirement, f.attempt)
	...
	t.Run("message-question-policy-bindings", func(t *testing.T) {
		...
		expectDialogueSQLReject(t, ctx, pool, "23514", `INSERT INTO verification_overrides(id,tenant_id,occurrence_id,requirement_id,attempt_id,client_request_id,expected_version,result_version,result_status,accepted,reason,actor_id) VALUES($1,$2,$3,$4,$5,'foreign',4,5,'pending',true,'Must fail','local-parent')`, uuid.NewString(), f.tenant, other.occurrence, f.requirement, f.attempt)
