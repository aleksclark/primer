...
func (s *SQLConfirmationStore) Issue(ctx context.Context, p ConfirmationPreview) (ConfirmationPreview, error) {
	...
	_, err = s.DB.Exec(ctx, `
		...
		(id,tenant_id,actor_id,handle_hash,action_kind,action_digest,action,summary,expires_at,run_id,tool_step)
		VALUES($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9,NULLIF($10,'')::uuid,$11)`,
		p.ID, p.TenantID, p.ActorID, handleHash(p.Handle), p.Action.Kind,
		decodeDigest(p.ActionDigest), action, p.Summary, p.ExpiresAt, p.RunID, p.ToolStep)
	if err != nil {
		return ConfirmationPreview{}, err
	}
	return p, nil
