...
const generating = process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1];
const document = generating
  ? JSON.parse(await readFile(contract, "utf8"))
  : { paths: {}, components: { schemas: {} } };

const KEYWORDS = new Set(["object", "class", "in", "out", "fun", "val", "var", "when", "package"]);
const IMPLEMENTED_EXTENSIONS = new Set(["x-maxBytes", "x-nonBlank", "x-equalFields", "x-uniqueNormalized"]);
const METADATA_EXTENSIONS = new Set(["x-manifest", "x-transport"]);

function fail(message) {
  throw new Error(message);
}

function rejectUnsupportedExtensions(value, path = "$") {
  if (Array.isArray(value)) {
    value.forEach((entry, index) => rejectUnsupportedExtensions(entry, `${path}[${index}]`));
    return;
  }
  if (!value || typeof value !== "object") return;
  for (const [key, child] of Object.entries(value)) {
    if (key.startsWith("x-") && !IMPLEMENTED_EXTENSIONS.has(key) && !METADATA_EXTENSIONS.has(key)) {
      fail(`unsupported contract constraint ${key} at ${path}`);
    }
    rejectUnsupportedExtensions(child, `${path}.${key}`);
  }
}
...
export { rejectUnsupportedExtensions, IMPLEMENTED_EXTENSIONS, METADATA_EXTENSIONS };

if (import.meta.url === `file://${process.argv[1]}`) {
  rejectUnsupportedExtensions(document);
