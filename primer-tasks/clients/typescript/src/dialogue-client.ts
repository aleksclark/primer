...
export interface DialogueClient { readonly conversationId: string; subscribe(listener: (snapshot: DialogueClientSnapshot) => void): () => void; connect(): void; disconnect(): void; sendMessage(text: string): string; snapshot(): DialogueClientSnapshot; }
const OPEN = 1;
...
export function createDialogueClient(options: DialogueClientOptions): DialogueClient {
  ...
  const connect = () => {
    ...
    try {
      ...
      socket.onmessage = (message) => { try { const event = parseStudentDialogueEvent(JSON.parse(message.data as string)); if (!event) throw new Error("invalid event"); if (event.kind !== "hello" && event.sequence > 0 && event.sequence <= current.cursor) return; current = { ...current, cursor: Math.max(current.cursor, event.cursor, event.sequence), events: [...current.events, ...project(event, options.attemptId ?? options.conversationId)].slice(-maxEvents) }; if (event.kind === "error") current.error = clientError("server", event.message ?? "The verifier reported an error.", event.retryable ?? false); notify(); } catch { state(current.connectionState, clientError("protocol", "The student verifier sent invalid data.", false)); } };
