...
// via streaming (child.RunText) and surfaces child start/text/tool/end events
// on the EventSink with root/parent/depth attribution.
//
// Unlike stock agenttool.New, this does NOT Collect() away intermediate updates.
// See spikes/maf-go/RUN_REPORT.md F3 for the negative evidence on stock agenttool.
type StreamingChildTool struct {
	child       *mafagent.Agent
	childType   string
	parentRunID string
	rootRunID   string
	depth       int
	sink        EventSink
	runOpts     []mafagent.Option
	// onChildRun optional hook for tests (e.g. capture child ctx).
...
func (t *StreamingChildTool) Call(ctx context.Context, args string) (any, error) {
	...
	// If child runner exists, record this as its last run id for lineage.
	...
	var runErr error
	for update, err := range t.child.RunText(ctx, in.Query, t.runOpts...) {
		if err != nil {
			runErr = err
...
// Ensure StreamingChildTool implements maftool.FuncTool at compile time.
var _ maftool.FuncTool = (*StreamingChildTool)(nil)
