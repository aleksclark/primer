...
func TestWriteShowNFO_Golden(t *testing.T) {
	...
	assert.Contains(t, s, "<lockdata>false</lockdata>", "allow first-import NFO metadata; remote providers are disabled on the source library")
...
func TestWriteEpisodeNFO_Golden(t *testing.T) {
	...
	assert.Contains(t, s, "<lockdata>false</lockdata>", "do not freeze the filename-derived title before NFO import")
