...
func TestWriteShowNFO_Golden(t *testing.T) {
	...
	assert.Contains(t, s, "<lockdata>true</lockdata>", "YouTube is not a TVDB-identifiable show")
...
func TestWriteEpisodeNFO_Golden(t *testing.T) {
	...
	assert.Contains(t, s, "<lockdata>true</lockdata>", "ledger ordinals must not fetch unrelated TV episode metadata")
